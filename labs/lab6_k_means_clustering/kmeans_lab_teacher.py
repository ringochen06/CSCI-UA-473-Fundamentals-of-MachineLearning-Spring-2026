...
for iteration in range(max_iters):
    ...
    new_centroids = np.array([
        data[labels == j].mean(axis=0) for j in range(k)
    ])
...
# Create and fit KMeans (k-means++ is the default init)
...
# Pick k based on the elbow analysis above
k = 5
emb_kmeans = KMeans(n_clusters=k, random_state=42, n_init=10)
emb_kmeans.fit(embeddings)
...
for j in range(k):
    ...
    answer = st.text_input(
        "Type your answer here to reveal the solution:", key="bonus_q_kmeans"
    )
    if answer:
        st.success(
            "**Answer:** K-means struggles with **elongated, overlapping, or differently-sized** clusters "
            "because it uses hard assignments and spherical distance. **Gaussian Mixture Models (GMMs)** "
            "handle this better by modeling each cluster as a Gaussian distribution with its own mean "
            "*and covariance* — allowing elliptical clusters of different shapes and sizes. GMMs also "
            "give **soft assignments** (probabilities of belonging to each cluster) instead of hard 0/1 labels."
        )
